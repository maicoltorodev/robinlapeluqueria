...
export function FramedImage({
  ...
  const borderClasses = borderSize === 8
    ? borderColor === "foreground"
      ...
      : "border-8 border-background/30 group-hover:border-background/50"
    : borderColor === "foreground"
      ? "border-[6px] border-foreground group-hover:border-foreground/80"
      : "border-[6px] border-background/30 group-hover:border-background/50"

  const insetClasses = borderSize === 8 ? "inset-[8px]" : "inset-[6px]"

  return (
    <div className={`relative ${height} group cursor-pointer`}>
      {/* Skeleton loader mientras carga */}
      <div className="absolute inset-0 bg-muted/30" />
      <div className={`absolute inset-0 ${borderClasses} transition-all duration-500 shadow-2xl`}>
        <div className={`absolute ${insetClasses} bg-gradient-to-br from-muted/50 to-background overflow-hidden`}>
          <Image
            src={src}
            alt={alt}
            fill
